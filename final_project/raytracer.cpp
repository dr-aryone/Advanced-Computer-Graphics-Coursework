...
Vec3f RayTracer::TraceRay(Ray &ray, Hit &hit, int bounce_count, double refractive_index) const {
  ...
  if(m->getTransparency()){
    ...
    /*
        ...
        O = Z x -N = right-orthovec
        O.normalize()
        
        theta2 = {stuff};
        
        double adj = cos(theta2);
        
        double opp = sin(theta2);
        
        final_vec = (opp * O) + (adj * -N);
    */
    ...
    answer += TraceRay(escapeRay, insideHit, bounce_count) * m->getTransparency(); //m->getRefractiveColor();
